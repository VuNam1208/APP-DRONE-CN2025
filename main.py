...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.replan = ReplanCoordinator(
            ...
            config=ReplanConfig(
                enabled=False,
                remap_mode="serpentine",  # B: remap all remaining → strips + lawnmower
                local_only=False,
                grid_spacing_m=0.0,  # auto from remaining WPs
            ),
            log_fn=self.log_replan,
        )
        self.setup_replan_controls()
    ...
    def log_replan(self, message):
        print(message)
        try:
            self.ui.file_all_uav.appendPlainText(message)
            self.ui.plainTextEdit_all_6_uav.appendPlainText(message)
            if hasattr(self, "replan_status_label"):
                self.replan_status_label.setText(message[:120])
        except Exception:
            pass

    def setup_replan_controls(self):
        # Insert into the left flight-control column layout (frame_10),
        # below START — avoid absolute geometry which overlays Take off / Land.
        layout = self.ui.verticalLayout_7
        parent = self.ui.frame_10

        # Allow a bit more height so new buttons fit without clipping.
        parent.setMaximumSize(QtCore.QSize(180, 560))
        parent.setMinimumSize(QtCore.QSize(180, 300))

        btn_style = (
            "QPushButton { min-height: 28px; max-height: 28px; border-radius: 4px; }"
        )

        self.btn_replan_toggle = QPushButton("Auto-Replan: OFF", parent)
        self.btn_replan_toggle.setStyleSheet(
            btn_style + "QPushButton { background-color: rgb(255, 170, 0); }"
        )
        self.btn_replan_toggle.clicked.connect(self.toggle_replan_enabled)

        self.btn_replan_force = QPushButton("Force Replan", parent)
        self.btn_replan_force.setStyleSheet(
            btn_style + "QPushButton { background-color: rgb(255, 85, 127); color: white; }"
        )
        self.btn_replan_force.clicked.connect(
            lambda: asyncio.create_task(self.force_replan_dialog())
        )

        self.btn_sim_fail = QPushButton("Sim Fail UAV1", parent)
        self.btn_sim_fail.setStyleSheet(
            btn_style + "QPushButton { background-color: rgb(85, 170, 255); }"
        )
        self.btn_sim_fail.clicked.connect(
            lambda: asyncio.create_task(self.simulate_uav_fail(1))
        )

        self.replan_status_label = QLabel("Replan standby (serpentine remap)", parent)
        self.replan_status_label.setWordWrap(True)
        self.replan_status_label.setStyleSheet(
            "color: rgb(0, 200, 0); background: transparent; font-size: 11px;"
        )
        self.replan_status_label.setMinimumHeight(28)

        layout.addWidget(self.btn_replan_toggle)
        layout.addWidget(self.btn_replan_force)
        layout.addWidget(self.btn_sim_fail)
        layout.addWidget(self.replan_status_label)

    def toggle_replan_enabled(self):
        enabled = not self.replan.config.enabled
        self.replan.set_enabled(enabled)
        base = "QPushButton { min-height: 28px; max-height: 28px; border-radius: 4px; }"
        if enabled:
            self.btn_replan_toggle.setText("Auto-Replan: ON")
            self.btn_replan_toggle.setStyleSheet(
                base + "QPushButton { background-color: rgb(0, 200, 0); color: white; }"
            )
        else:
            self.btn_replan_toggle.setText("Auto-Replan: OFF")
            self.btn_replan_toggle.setStyleSheet(
                base + "QPushButton { background-color: rgb(255, 170, 0); }"
            )

    async def simulate_uav_fail(self, index: int = 1):
        from replan_coordinator import load_waypoints_from_plan

        self.replan.states[index].battery_percent = 5.0
        self.replan.states[index].mode = "RTL"
        self.replan.states[index].in_mission = True
        if self.replan.states[index].lat is None:
            wps = load_waypoints_from_plan(self.mission_plan_path(index))
            if wps:
                self.replan.states[index].lat = wps[0][0]
                self.replan.states[index].lon = wps[0][1]
        if self.replan.states[index].lat is None:
            self.log_replan(
                f"[REPLAN] Sim Fail aborted: UAV {index} has no GPS/plan "
                f"({os.path.basename(self.mission_plan_path(index))})"
            )
            return
        self.replan.states[index].connected = True
        self.replan.states[index].last_telemetry_s = time.time()
        self.replan.states[index].failed = True

        alive_ready = []
        for other in self.replan.config.monitor_indices:
            if other == index:
                continue
            st = self.replan.states[other]
            st.failed = False
            # Only treat drones with a mission plan (or known GPS) as alive helpers.
            if st.lat is None or st.lon is None:
                wps = load_waypoints_from_plan(self.mission_plan_path(other))
                if wps:
                    st.lat, st.lon = wps[0]
                    st.last_telemetry_s = time.time()
            if st.lat is None or st.lon is None:
                st.connected = False
                continue
            st.connected = True
            st.in_mission = True
            alive_ready.append(other)

        if not alive_ready:
            self.log_replan(
                "[REPLAN] Sim Fail aborted: no other UAV has plan/GPS to accept remap"
            )
            return

        self.log_replan(
            f"[REPLAN] Simulated failure on UAV {index}; alive helpers={alive_ready}"
        )
        if not self.replan.config.enabled:
            self.toggle_replan_enabled()
        await self.execute_replan_for_failure(index)

    async def force_replan_dialog(self):
        from replan_coordinator import load_waypoints_from_plan

        candidate = None
        for index in self.replan.config.monitor_indices:
            if self.replan.states[index].failed:
                candidate = index
                break
        if candidate is None:
            candidate = 1
            self.replan.states[candidate].failed = True
            self.replan.states[candidate].in_mission = True
            if self.replan.states[candidate].lat is None:
                wps = load_waypoints_from_plan(self.mission_plan_path(candidate))
                if wps:
                    self.replan.states[candidate].lat = wps[0][0]
                    self.replan.states[candidate].lon = wps[0][1]
            # Seed other drones that have plans so remap has helpers.
            for other in self.replan.config.monitor_indices:
                if other == candidate:
                    continue
                st = self.replan.states[other]
        ...
        if self.replan.states[candidate].lat is None:
            self.log_replan(
                f"[REPLAN] Force Replan aborted: UAV {candidate} has no GPS/plan"
            )
            return
        self.log_replan(f"[REPLAN] Manual force replan using failed UAV {candidate}")
        await self.execute_replan_for_failure(candidate)
            ...
            try:
                if self.replan.can_replan_now():
                    ...
                    for failed_index in failed:
                        await self.execute_replan_for_failure(failed_index)
                        break
            except Exception as exc:
                self.log_replan(f"[REPLAN] monitor error: {exc}")
    ...
    async def execute_replan_for_failure(self, failed_index: int):
        ...
        try:
            self.replan.states[failed_index].failed = True
            self.replan.states[failed_index].in_mission = False
            result = self.replan.build_replan(failed_index)
            if result is None:
                self.log_replan(
                    f"[REPLAN] no replan produced for UAV {failed_index}; "
                    "fail latch kept (use Force Replan to retry)"
                )
                return

            result.alive_indices = [
                i for i in result.alive_indices if len(result.routes.get(i, [])) >= 1
            ]
            if not result.alive_indices:
                self.log_replan("[REPLAN] no valid alive routes; fail latch kept")
                return

            # Pause every connected helper first; only those with new routes restart.
            helpers = [
                i
                for i in self.replan.config.monitor_indices
                if i != failed_index
                and not self.replan.states[i].failed
                and self.replan.states[i].connected
            ]
            for index in helpers:
                try:
                    await self.pause_drone(index)
                except Exception:
                    pass

            try:
                if self.replan.states[failed_index].connected:
                    await self.rtl_drone(failed_index)
                    self.log_drone(failed_index, "[REPLAN] RTL requested after failure")
            except Exception as exc:
                self.log_replan(f"[REPLAN] RTL failed UAV {failed_index}: {exc}")

            start_t = time.time()
            restarted = set()
            for index in result.alive_indices:
                delay = float(result.delays.get(index, 0.0))
                alt = float(result.altitudes.get(index, 10.0))
                try:
                    getattr(self.ui, f"edit_high_drone_{index}").setPlainText(str(round(alt, 1)))
                except Exception:
                    pass
                self.log_drone(
                    index,
                    f"[REPLAN] new route points={len(result.routes.get(index, []))} "
                    f"alt={alt:.1f}m delay={delay:.1f}s "
                    f"file={os.path.basename(result.plan_files.get(index, ''))}",
                )
                wait_s = delay - (time.time() - start_t)
                if wait_s > 0:
                    await asyncio.sleep(wait_s)
                try:
                    await self.upload_and_restart_mission(index)
                    self.replan.mark_in_mission(index, True)
                    restarted.add(index)
                except Exception as exc:
                    self.log_replan(f"[REPLAN] upload/start UAV {index} failed: {exc}")

            # Helpers that did not receive a remapped route: resume previous mission.
            for index in helpers:
                if index in restarted:
                    continue
                try:
                    await self.get_drone(index).mission.start_mission()
                    self.replan.mark_in_mission(index, True)
                    self.log_drone(index, "[REPLAN] no new strip; resumed previous mission")
                except Exception as exc:
                    self.log_replan(f"[REPLAN] resume UAV {index} failed: {exc}")

            self.log_replan(
                f"[REPLAN] done | conflicts {result.conflicts_before}->{result.conflicts_after} | "
                f"remaining_points={result.remaining_points} | restarted={sorted(restarted)}"
            )
        finally:
            self.replan.end_replan()

    async def upload_and_restart_mission(self, index: int):
        drone = self.get_drone(index)
        mission_import = await self.import_mission(index)
        if mission_import is None:
            raise RuntimeError(f"No mission plan for drone {index}")
        speed = self.mission_speed(index)
        try:
        ...
        self.log_drone(index, "[REPLAN] mission restarted")
    ...
    async def connect_drone(self, index):
        ...
        self.replan.mark_connected(index, True, clear_failed=False)
    ...
    async def start_mission_drone(self, index):
        ...
        async for mission_progress in drone.mission.mission_progress():
            if mission_progress.current == mission_progress.total:
                self.replan.mark_in_mission(index, False)
    ...
    async def get_alt(self, index):
        ...
        async for position in drone.telemetry.position():
            ...
            self.replan.update_position(index, latitude, longitude, alt_rel)
    ...
    async def get_mode(self, index):
        async for mode in self.get_drone(index).telemetry.flight_mode():
            ...
            self.replan.update_mode(index, mod)

    async def get_batt(self, index):
        async for batt in self.get_drone(index).telemetry.battery():
            v = round(batt.voltage_v, 1)
            rem_raw = float(batt.remaining_percent)
            # MAVSDK usually reports 0..1; some setups already give 0..100.
            rem = rem_raw * 100.0 if rem_raw <= 1.5 else rem_raw
            rem = round(max(0.0, min(rem, 100.0)), 1)
            getattr(self.ui, f"Batt_V_uav{index}").setText(f"{v} V")
            getattr(self.ui, f"Batt_Rem_uav{index}").setText(f"{rem} %")
            self.replan.update_battery(index, rem)
